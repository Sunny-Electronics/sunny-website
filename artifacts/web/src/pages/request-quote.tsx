...
export default function RequestQuote() {
  ...
  useEffect(() => {
    if (appliedPrefillRef.current || typeof window === "undefined") return;

    const params = new URLSearchParams(window.location.search);
    if (params.get("family") !== "crystal") return;
    appliedPrefillRef.current = true;

    const packageCode = (params.get("pkg") || "R").toUpperCase();
    const packageOption = crystalPackages.find((item) => item.code === packageCode) ?? crystalPackages.find((item) => item.code === "R")!;
    const frequencyValue = params.get("frequency") || "32";
    const capacitanceValue = (params.get("cl") || "12").replace(/[^\d.]/g, "") || "12";
    const toleranceValue = (params.get("tolerance") || "50").replace(/[^\d]/g, "") || "50";
    const tempCode = (params.get("temp") || "E").toUpperCase();
    const tempOption = crystalTempOptions.find((item) => item.code === tempCode) ?? crystalTempOptions.find((item) => item.code === "E")!;
    const stabilityCode = params.get("stability") || "7";
    const stabilityOption = stabilityOptions.find((item) => item.code === stabilityCode) ?? stabilityOptions.find((item) => item.code === "7")!;
    const modeValue = params.get("mode") || "1";
    const quantityValue = formatQuantity(params.get("quantity") || "10000") || "10,000";
    const noteValue = params.get("note") || "Generated by Sunny chat for RFQ review.";
    const packingValue = params.get("packing") || "TR";
    const generatedCrystalPart = `S${packageOption.code}${capacitanceValue}${modeValue}${toleranceValue}${tempOption.code}${stabilityOption.code}-${formatMHz(frequencyValue)}${packingSuffix(packingValue)}`;
    const generatedSpec = `${toleranceValue}/${stabilityOption.catalogValue} ${tempOption.label}/${capacitanceValue}pF/${findPackingLabel(packingValue)}`;

    setFamily("crystal");
    setFamilyMenuOpen(false);
    setCrystalPackage(packageOption.code);
      ...
      setQuoteLines([
        {
          ...
          partNumber: generatedCrystalPart,
          packageType: packageOption.type,
          frequency: `${formatMHz(frequencyValue)} MHz`,
          spec: generatedSpec,
